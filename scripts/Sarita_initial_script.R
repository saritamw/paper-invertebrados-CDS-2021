...
## BOXPLOTS POR MONITOREO

colnames(pest_data)

par(mar=c(4.3, 4.1, 0.8, 2.1),mfrow=c(2,2))
boxplot(especie~monitoreo, data=pest_data, xlab="Monitoreos", ylab="N° de especies")
boxplot(especie~trampa_ID,data=pest_data,xlab="Trampas", ylab="N° de especies")
boxplot(abundancia_trampa~monitoreo,data=pest_data,xlab="Monitoreos", ylab="Abundancia")
boxplot(abundancia_trampa~trampa_ID, data=pest_data, xlab="Trampas", ylab="Abundancia")

test_table <- table(pest_data$max_temp, pest_data$monitoreo)
test_table
plot(pest_data$monitoreo, pest_data$max_temp)

table1 <- table(pest_data$num_trampa_monitoreo, pest_data$abundancia_trampa)
table1

par(mfrow=c(1,1))
boxplot(pest_data$num_trampa_monitoreo, pest_data$abundancia_trampa)

tabl2 <- table(pest_data$abundancia_trampa, pest_data$ubicacion, pest_data$periodo_monitoreo)
tabl2
?group_by()
